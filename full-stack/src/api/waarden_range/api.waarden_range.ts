...
export const getWaardenRangeByUnitId = async (unitId: string): Promise<WaardenRange | null> => {
  const { data, error } = await API.from("waarden_range").select("*").eq("unit_id", unitId).single();
  if (error) {
