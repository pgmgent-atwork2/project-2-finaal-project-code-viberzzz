...
export const getRapporten = async (): Promise<Rapport[] | null> => { 
    const rapporten = await Promise.all([
      API.from("filtratie_unit")
        .select("*, waarden_range(*), filtratie_waarden(*, medewerker:medewerker_id(naam)), onderhoud(*, gebruiker:toegewezen_aan(naam))")
    ]);
      ...
      .flatMap((r) => {
        return r.data!.flatMap((unit) => {
          ...
          return unit.filtratie_waarden.map((waarden: any) => {
            // Get the closest onderhoud by date, or the latest one
            const closestOnderhoud = unit.onderhoud && unit.onderhoud.length > 0
              ? unit.onderhoud.sort((a: Onderhoud, b: Onderhoud) => {
                  const dateA = new Date(a.start_datum || a.bijgewerkt_op).getTime();
                  const dateB = new Date(b.start_datum || b.bijgewerkt_op).getTime();
                  const measureDate = new Date(waarden.gemeten_op).getTime();
                  // Find closest to measurement date
                  return Math.abs(dateA - measureDate) - Math.abs(dateB - measureDate);
                })[0]
              : null;
            ...
            if (unit.waarden_range) {
              const hasAllData = waarden.ph !== null && 
            ...
            const rapport: Rapport = {
              ...
              technicus: waarden.medewerker || { naam: 'Onbekend' } as Gebruiker,
...
export const getTemperatuurTrend = async (unitId: string): Promise<temperatuurTrend[] | null> => {
    let query = API
        .from("filtratie_waarden")
...
export const getWaterniveauTrend = async (unitId: string): Promise<waterniveauTrend[] | null> => {
    let query = API
        .from("filtratie_waarden")
...
export const getOnderhoudTrend = async (unitId: string): Promise<onderhoudTrend[] | null> => {
    let query = API
        .from("onderhoud")
